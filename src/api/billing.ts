...
import type { BillingCatalogPlan, BillingInvoice, BillingPlanDetail, BillingProfileInput, Tenant } from "./types";
...
export async function getBillingPlanDetail(id: string) { const { data } = await http.get<BillingPlanDetail>(`/admin/billing/plans/${id}`); return data; }
...
/** Platform-geniş: bir dövrün bütün müəssisələrinin fakturaları - "kim ödəməyib" siyahısı. */
export async function listAllInvoices(period?: string) { const { data } = await http.get<BillingInvoice[]>("/admin/billing/invoices", { params: period ? { period } : undefined }); return data; }
